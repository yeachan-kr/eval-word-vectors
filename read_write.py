...
def read_word_vectors(filename, v, dimension, oov):
    emb = pickle.load(open(filename, 'rb'))
    word_vecs = {}

    for w in v:
        if w in emb:
            word_vecs[w] = emb[w] / math.sqrt((emb[w]**2).sum() + 1e-6)
        elif oov == 1:
            word_vecs[w] = numpy.zeros((dimension))
